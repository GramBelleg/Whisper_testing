...
describe('editing profile', () => {
  ...
  const test_Email = "isadore.torphy@gmail.com" 
  const test_password = "Abcdefgh12#"
  let selectors;
  before(() => {
  ...
  describe('phone number', () => {
    ...
    it('change profile picture', () => {
      ...
      cy.get('[data-testid="EditProfilePic"]').click(); // Replace with the actual button selector
  
      // Upload a new picture
      const filePath = 'profile.png'; // Path to your test image
      cy.get('input[type="file"]').attachFile(filePath); // Requires the `cypress-file-upload` plugin
  
      // Confirm the upload
      // cy.get('#upload-confirm-btn').click(); // Replace with the actual confirm button selector
  
      // Verify the profile picture has been updated
      cy.get('.absolute')
        .should('have.attr', 'src')
        .and('include', 'newProfilePic.jpg'); // Ensure the new picture is displayed 
